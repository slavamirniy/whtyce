...
function renderTerminalImage(text) {
    const canvas = (0, canvas_1.createCanvas)(IMG_WIDTH, IMG_HEIGHT);
...
function listTmuxSessions() {
    try {
        return (0, child_process_1.execSync)("tmux list-sessions -F '#{session_name}'", { encoding: 'utf-8' })
            .trim().split('\n').filter(Boolean);
    }
    catch {
        return [];
...
function createTmuxSession(name, cwd) {
    try {
        (0, child_process_1.execSync)(`tmux new-session -d -s ${JSON.stringify(name)} -x ${TERM_COLS} -y ${TERM_ROWS} -c "${cwd || process.cwd()}"`, {
            encoding: 'utf-8',
        });
        return true;
    }
...
class TmateTelegramBot {
    constructor(config) {
        ...
        this.bot = new node_telegram_bot_api_1.default(config.token, { polling: true, filepath: true });
    ...
    isConnected() { return this.authorizedChat !== null; }
    ...
    registerCommands() {
        ...
        if (this.config.threadsEnabled) {
            commands.push({ command: 'new', description: 'Create tmux session (/new [name])' }, { command: 'kill', description: 'Kill tmux session (/kill <name>)' }, { command: 'sessions', description: 'Refresh session list' });
        }
    ...
    setupHandlers() {
        ...
        this.bot.on('message', async (msg) => {
            ...
            if (text.startsWith('//')) {
                ...
                if (this.config.threadsEnabled && msg.message_thread_id) {
                    ...
                    if (ts && this.activeSession !== ts)
                        this.activateSession(ts);
                }
                if (targetSession) {
        ...
        this.bot.on('callback_query', async (query) => {
            ...
            if (action && this.activeSession) {
                if (action[0])
                    this.sendToTmuxSpecial(this.activeSession, action[0]);
    ...
    persistThreadIds() {
        ...
        for (const [name, state] of this.sessions) {
            if (state.threadId)
                ids[name] = state.threadId;
        }
        this.config.onThreadsChanged(ids);
    }
    getSessionForThread(threadId) {
        if (!threadId || !this.config.threadsEnabled)
            return null;
        for (const [name, state] of this.sessions) {
    ...
    async sendSessionPreview(sessionName) {
    ...
    async updateSessionKeyboard(sessionName) {
        const state = this.sessions.get(sessionName);
        if (!state?.screenMsgId || !this.authorizedChat)
            return;
        try {
    ...
    startContentPolling() {
        ...
        this.pollInterval = setInterval(() => {
            if (!this.activeSession || !this.authorizedChat)
                return;
    ...
    scheduleRender() {
        if (this.renderScheduled || this.updating)
            return;
        const now = Date.now();
    ...
    async doRender() {
        if (!this.activeSession || !this.authorizedChat || this.updating)
            return;
        this.updating = true;
        ...
        try {
            ...
            if (state.screenMsgId) {
                try {
                    ...
                    fs_1.default.writeFileSync(tmpFile, imgBuf);
                    try {
    ...
    async sendNewScreenshot(sessionName) {
        const state = this.sessions.get(sessionName);
        if (!state || !this.authorizedChat)
            return;
        try {
    ...
    async handleVoice(chatId, msg, targetSession) {
        ...
        const statusOpts = {};
        if (threadId)
            statusOpts.message_thread_id = threadId;
    ...
    sendToTmux(session, text) {
        const child = (0, child_process_1.spawn)('tmux', ['send-keys', '-t', session, '-l', text]);
        child.on('error', () => { });
    }
    sendToTmuxSpecial(session, key) {
        const child = (0, child_process_1.spawn)('tmux', ['send-keys', '-t', session, key]);
        child.on('error', () => { });
    }
